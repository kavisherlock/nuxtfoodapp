        ...
        <table>
          ...
          <tbody>
            <tr v-for="item in cart" :key="item.id">
              <td class="item">
                {{ item.item }}
                <span v-if="item.selectedOption && item.selectedOption.length">- {{ item.selectedOption }}</span>
                <div v-if="item.selectedAddOns && item.selectedAddOns.length">({{ item.selectedAddOns.join(', ') }})</div>
              </td>
              <td>{{ item.quantity }}</td>
              <td>{{ priceFormatting(item.quantity * item.price) }}</td>
            </tr>
            <tr>
              <td colspan="2"></td>
              <td class="total">Total: {{ cartTotal }}</td>
            </tr>
          </tbody>
        </table>
      </section>
